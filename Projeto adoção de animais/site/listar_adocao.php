                        ...
                        while ($linha = mysqli_fetch_array($resultados)) {
                           ...
                           // $nome_adotante = $linha['nome_adotante'];
                            //$nome_animal = $linha['nome_animal'];
                                    ...
                                    $sql2 = "SELECT * FROM tb_adotante WHERE id = $id_adotante";

                                    $resultados2 =  mysqli_query($conexao, $sql2);
        
                                    if (mysqli_num_rows($resultados2) > 0) {
                                        while ($linha2 = mysqli_fetch_array($resultados2)) {
        
                                            $nome_adotante = $linha2['nome'];
                                        }
                                    }
                                    $sql3 = "SELECT * FROM tb_animal WHERE id_animal = $id_animal";

                                    $resultados3 =  mysqli_query($conexao, $sql3);

                                    if (mysqli_num_rows($resultados3) > 0) {
                                        while ($linha3 = mysqli_fetch_array($resultados3)) {

                                    $nome_animal = $linha3['nome_animal'];
                                    ...
                                    echo "<h3>$nome_adotante</h3>";
                                    echo "<p>$id_adotante</p>";
                                    echo "<p>$nome_animal</p>";
                                    echo "</div>";
